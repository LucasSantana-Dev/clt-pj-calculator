...
- **Staging**: gate via hub central de auth (repo Criativaria-Projects/criativaria-auth, worker em https://auth.criativaria.workers.dev; vira auth.criativaria.com na Fase B do ADR-0003). O site manda para `<hub>/login?to=<url>`; o hub faz o OAuth do Discord (um único redirect URI `<hub>/callback` para todos os stagings), confere a guild e devolve token SSO de 60s que `/api/auth/sso` troca por sessão local de 7 dias. Secrets do site: STAGING (interruptor), AUTH_HUB_URL, SSO_SECRET, SESSION_SECRET. Novo site de staging = adicionar origem em ALLOWED_ORIGINS do hub + copiar functions/. Lançamento público: deletar a secret STAGING **e redisparar o deploy** (secrets de Pages só valem no próximo deployment, diferente de Workers — `gh run rerun` do último CI verde basta; incidente 2026-07-10). Verificar com `curl` que o título não contém "Staging".
